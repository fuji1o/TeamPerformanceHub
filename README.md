...
cd backend
python -m uvicron main:app --reload
...
7. НАСТРОЙКА МАППИНГА ПОЛЬЗОВАТЕЛЕЙ
====================================

Если один разработчик в GitLab фигурирует под разными именами (например, Olesya Shiryaeva
в коммитах и Liccva в MR), их нужно объединить.

8. Создайте файл author_mapping.json в папке backend:
...
===============================
10.1 Данные не загружаются в браузере
--------------------------------------
Откройте страницу в режиме инкогнито (Ctrl+Shift+N) — расширения браузера могут
блокировать запросы к API (AdBlock, uBlock Origin и др.)

10.2 График не отображается
----------------------------
Убедитесь, что у выбранного разработчика есть коммиты за указанный период.
Попробуйте выбрать период 90 дней.

10.3 Пустой список разработчиков
---------------------------------
Проверьте, что в проекте есть коммиты за выбранный период.
