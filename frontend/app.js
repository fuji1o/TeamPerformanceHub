...
function formatDuration(h) {
    ...
    const totalMinutes = Math.max(1, Math.round(h * 60));
...
const SIGNALS_CONFIG = [
    ...
    },
    {
        ...
        compute(r, username) {
            ...
            const avgText = formatDuration(avg);

            return {
                status: "neutral",
                badge: `<span class="stat-highlight">${avgText}</span>`,
                hint: `Среднее время до первого комментария: ${avgText}`
            };
        },
    },
    {
...
function renderMRTable(mrs) {
    ...
    tbody.innerHTML = mrs.map(mr => {
        ...
        const commentsCount = mr.comment_stats?.total ?? 0;
        ...
        <tr>
            ...
            <td>${commitsCount}</td>
            <td>${commentsCount > 0 ? `💬 ${commentsCount}` : '—'}</td>
            <td>${mr.time_to_merge_hours != null ? formatDuration(mr.time_to_merge_hours) : "—"}</td>
        </tr>`;
    }).join('');
}
